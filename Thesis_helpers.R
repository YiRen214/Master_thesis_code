...
##############################################
##############################################
####helpers(originally from: irdpackage)######
##############################################
...
evaluate_box = function(box, x_interest, predictor, predictor_2,n_samples, desired_range, strategy = "random") {
  yhat_interest = abs(predictor$predict(x_interest)[[1]]-predictor_2$predict(x_interest)[[1]])
  ## generate new data
  if (strategy == "random") {
  ...
  dt$pred = abs(predictor$predict(dt)-predictor_2$predict(dt))
  ## reuse generated one
...
make_surface_plot = function(box, param_set, grid_size, predictor,predictor_2, x_interest, feature_names, surface = "prediction", desired_range = NULL) {
  
  param_set_sub = param_set$clone()$subset(feature_names)
  dt_grid = make_ice_curve_area(predictor,predictor_2, x_interest, grid_size, param_set_sub, surface = surface, desired_range = desired_range)
  ...
  if (param_set_sub$all_numeric) {
    ...
    dt_grid$pred = abs(predictor$predict(dt_grid)[[1]]-predictor_2$predict(dt_grid)[[1]])
    y_hat_interest = abs(predictor$predict(x_interest)[[1]]-predictor_2$predict(x_interest)[[1]])
    x_interest_with_pred = cbind(x_interest, pred = y_hat_interest)
...
make_ice_curve_area = function(predictor,predictor_2, x_interest, grid_size, ps, surface, desired_range) {
  ...
  if (surface == "prediction") {
    pred = abs(predictor$predict(grid_dt)[[1]]-predictor_2$predict(grid_dt)[[1]])
  } else if (surface == "range") {
    pred = predict_range(predictor,predictor_2, newdata = grid_dt, range = desired_range)
  }
  cbind(grid_dt, pred)
}
...
predict_range = function(predictor, predictor_2,newdata, range) {
  prediction = abs(predictor$predict(newdata)-predictor_2$predict(newdata))[[1]]
...
get_max_box = function (x_interest, fixed_features, predictor, predictor_2,param_set, desired_range, resolution = 500L) {
...
sampling = function(predictor,predictor_2, x_interest, fixed_features, desired_range, param_set, num_sampled_points = 1000L, strategy = "traindata") {
  
  largest_box = get_max_box(x_interest = x_interest, fixed_features = fixed_features,
                            predictor = predictor,predictor_2=predictor_2, param_set = param_set, desired_range = desired_range)
